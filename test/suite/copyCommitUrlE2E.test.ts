...
suite('Copy Commit URL E2E Tests', () => {
  ...
  suiteSetup(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'git-history-copy-url-'));
  ...
  test('context menu has copy-url item', async () => {
    const mainJsPath = path.resolve(__dirname, '../../../src/webview/panel/main.js');
    const source = fs.readFileSync(mainJsPath, 'utf-8');
  ...
  test('context menu click handles copy-url action', async () => {
    const mainJsPath = path.resolve(__dirname, '../../../src/webview/panel/main.js');
    const source = fs.readFileSync(mainJsPath, 'utf-8');

    assert.ok(source.includes("action === 'copy-url'"),
      'Should handle copy-url action');
  });
  ...
  test('parseRemoteUrl handles different URL formats', async () => {
    ...
    // After refactoring, parseRemoteUrl delegates to detectPlatform() for platform detection
    // So we check the full gitService.ts source which contains both functions
    assert.ok(source.includes('github.com'),
      'Should handle github.com URLs');
    assert.ok(source.includes('gitlab.com'),
      'Should handle gitlab.com URLs');
    assert.ok(source.includes('bitbucket.org'),
  ...
  test('platform detection function exists', async () => {
    ...
    assert.ok(source.includes('function detectPlatform'),
      'detectPlatform function should exist');
