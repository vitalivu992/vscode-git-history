...
suite('Author Sort E2E Tests', () => {
  ...
  test('graph is hidden for author sort modes', () => {
    const mainJsPath = path.resolve(__dirname, '../../../src/webview/panel/main.js');
    const source = fs.readFileSync(mainJsPath, 'utf-8');

    assert.ok(source.includes('effectiveShowGraph'), 'Should compute effectiveShowGraph');
    assert.ok(source.includes('sortMode < 2'), 'Graph should only show for modes 0 and 1');
  });
