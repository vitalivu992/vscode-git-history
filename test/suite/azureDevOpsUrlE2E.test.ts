...
suite('Azure DevOps URL E2E Tests', () => {
  ...
  test('getBranchUrl handles Azure DevOps platform', async () => {
    ...
    assert.ok(fnStart >= 0, 'getBranchUrl should exist');
    const fnEnd = source.indexOf('\n}', fnStart);
    const fnBody = source.substring(fnStart, fnEnd);

    assert.ok(fnBody.includes("'azure'"),
      'getBranchUrl should handle Azure DevOps platform');
    assert.ok(fnBody.includes('version=GB'),
      'Should use version=GB parameter for Azure DevOps branches');
  });
  ...
  test('getFileUrl handles Azure DevOps platform', async () => {
    ...
    assert.ok(fnStart >= 0, 'getFileUrl should exist');
    const fnEnd = source.indexOf('\n}', fnStart);
    const fnBody = source.substring(fnStart, fnEnd);

    assert.ok(fnBody.includes("'azure'"),
      'getFileUrl should handle Azure DevOps platform');
    assert.ok(fnBody.includes('path='),
      'Should use path parameter for Azure DevOps files');
  });
