...
suite('Clear All Filters Source Verification Tests', () => {
  ...
  test('types.ts includes clearAllFilters in WebviewToExtMessage', () => {
    const source = fs.readFileSync(typesPath, 'utf-8');
    assert.ok(source.includes("type: 'clearAllFilters'"), 'types.ts should include clearAllFilters message type');
  });
