...
suite('Toggle Hide Merge Commits E2E Source Integration Tests', () => {
  ...
  test('merge toggle button triggers handleMergeToggle on click', () => {
    ...
    assert.ok(mainSource.includes('merge-toggle-btn'), 'init should bind event to merge-toggle-btn');
    assert.ok(mainSource.includes('handleMergeToggle'), 'init should call handleMergeToggle on button click');
  });
});
