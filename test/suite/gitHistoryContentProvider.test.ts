...
suite('GitHistoryContentProvider Behavioral Tests', () => {
  ...
  test.skip('provideTextDocumentContent returns content from specific commit', async () => {
    const { execSync } = require('child_process');
    const commitsOutput = execSync('git log --format=%H', { cwd: tempDir, encoding: 'utf-8' });
    const hashes = commitsOutput.trim().split('\n');
  ...
  test.skip('provideTextDocumentContent returns different content for different commits', async () => {
    const { execSync } = require('child_process');
    const commitsOutput = execSync('git log --format=%H', { cwd: tempDir, encoding: 'utf-8' });
    const hashes = commitsOutput.trim().split('\n');
