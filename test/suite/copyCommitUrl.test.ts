...
suite('Copy Commit URL Test Suite', () => {
  ...
  test('context menu should have copy-commit-url item', () => {
    const source = fs.readFileSync(mainJsPath, 'utf-8');
  ...
  test('context menu click handler should handle copy-commit-url action', () => {
    const source = fs.readFileSync(mainJsPath, 'utf-8');
    assert.ok(source.includes("action === 'copy-commit-url'"),
      'Context menu click handler should handle copy-commit-url action');
    const handlerIdx = source.indexOf("action === 'copy-commit-url'");
    const nearby = source.substring(handlerIdx, handlerIdx + 200);
    assert.ok(nearby.includes('handleCopyUrl'),
      'copy-commit-url handler should call handleCopyUrl');
  });
