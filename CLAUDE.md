...
- **Copy File Line Count**: Press `Ctrl+Shift+Alt+L` / `Cmd+Shift+Alt+L` to copy the number of lines in a file at the selected commit to clipboard. Outputs "N line" or "N lines" (singular/plural). Useful for tracking code complexity and file growth over time. The `handleCopyFileLineCount` function in `main.js` sends a `copyFileLineCount` message with the file path and commit hash. The message is handled by `handleCopyFileLineCount` in `messageHandler.ts` which uses `getFileContentAtCommit()` from `gitService.ts` to fetch the file content, counts lines by splitting on newlines, and writes the count to `vscode.env.clipboard`. Shows "Unable to get file content" when the file cannot be read. Also available in the file context menu. The `copyFileLineCount` message type and `copyFileLineCount` webview action are defined in `src/types.ts`.
